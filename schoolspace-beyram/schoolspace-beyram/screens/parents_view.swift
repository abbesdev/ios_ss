...
@available(iOS 13.0, *)
public struct Parents_view : View {
...
@available(iOS 13.0, *)
struct SlidingTabConsumerView2 : View {
    @State private var selectedTabIndex = 0
    @State var parents: [User] = []
        ...
        VStack(alignment: .leading) {
            ...
            ScrollView {
                ForEach(parents) { parent in
                    if (selectedTabIndex == 0 && parent.verified && parent.userRole == "parent")
                    {
                        ListViewCell2(name: "\(parent.firstName) \(parent.lastName)", email: parent.email, number:"#\(parent.registrationCode)", _id : parent._id)
                       

                            
                    }else if (selectedTabIndex == 1 && !parent.verified && parent.userRole == "parent"){
                        ListViewCell2(name: "\(parent.firstName) \(parent.lastName)", email: parent.email, number:"#\(parent.registrationCode)", _id : parent._id)
    ...
    func fetchData() {
          guard let url = URL(string: "http://172.17.7.17:8080/users") else { return }
...
struct SlidingTabView_Previews2 : PreviewProvider {
