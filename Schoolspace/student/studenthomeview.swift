...
struct studenthomeview: View {
     ...
     //   Text(/*@START_MENU_TOKEN@*/"Hello, World!"/*@END_MENU_TOKEN@*/)
        NavigationView{
            VStack( spacing: 2){
                ...
                Text("Upcoming events").font(.subheadline).frame(maxWidth: .infinity, alignment: .leading)
                    .padding()
    ...
    func loadData() {
        guard let url = URL(string: "\(api.baseurl)/students/\(studentID)") else {
            return
        }
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            do {
                let countsResponse = try JSONDecoder().decode(Student1.self, from: data)
    ...
    func getAllEvents(completion: @escaping ([Event1]?, Error?) -> Void) {
        guard let url = URL(string: "\(api.baseurl)/event") else {
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            if httpResponse.statusCode == 200 {
                do {
                    ...
                    let events = try JSONDecoder().decode([Event1].self, from: data)
