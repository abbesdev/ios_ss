...
struct quizview: View {
 ...
 //   @State private var selectedAnswers = [String]()
    ...
    @State private var questions: [Question1] = []
   @State private var answer: [String] = []
    ...
    func submitQuiz() {
        ...
        let url = URL(string: "\(api.baseurl)/quiz/\(quizId)/\(studentID)")!
        print("quizid")
        print(quizId)
   ...
   /* func getQuestions(quizId: String) async throws -> [Question] {
    ...
    func getQuestions(quizId: String, completion: @escaping ([Question1]?, Error?) -> Void) {
        print(quizId)
        guard let url = URL(string: "\(api.baseurl)/questionsbyquiz/\(quizId)") else {
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            do {
                let questions = try JSONDecoder().decode([Question1].self, from: data)
