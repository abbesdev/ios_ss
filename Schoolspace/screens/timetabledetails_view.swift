...
struct timetabledetails_view: View {
    ...
    @State private var opacitytext = 0
    ...
    @State var showfirstwarning = false
    @State var showsecondwarning = false
    ...
    @State var classOptions: [Classes1] = []
    @State var subjectOptions: [Subject1] = []
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        Text("Date")
                            ...
                            DatePicker(selection: $selectedDate, displayedComponents: [.date], label: { Text("") })
                                ...
                                .background(Color.gray.opacity(1))
                                .cornerRadius(8)
                        if self.showfirstwarning{
                            Text("*Incorrect Date ").foregroundColor(Color.red)
                            .bold()}
                        ...
                        VStack{
                            Text("End time")
                                ...
                                DatePicker(selection: $endtime, displayedComponents: [.hourAndMinute], label: { Text("") })
                                    ...
                                    .background(Color.gray.opacity(1))
                                    .cornerRadius(8)
                            if self.showsecondwarning{
                                Text("*Incorrect Time ").foregroundColor(Color.red)
                                .bold()}
    ...
    func addClass() {
        // Construct the URL
        let url = URL(string: "\(api.baseurl)/timetable")!
    ...
    func formatDate(_ date: Date, time: Date) -> String {
        ...
        formatter.timeZone = TimeZone(secondsFromGMT: 0) // Set the time zone to UTC
    ...
    func getClasses() {
        guard let url = URL(string: "\(api.baseurl)/class") else {
            print("Invalid URL")
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data {
                if let decodedResponse = try? JSONDecoder().decode([Classes1].self, from: data) {
    ...
    func getSubjects() {
        guard let url = URL(string: "\(api.baseurl)/subject") else {
            print("Invalid URL")
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data {
                if let decodedResponse = try? JSONDecoder().decode([Subject1].self, from: data) {
                    DispatchQueue.main.async {
